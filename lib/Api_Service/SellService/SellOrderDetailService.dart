...
class SellOrderDetailService {
  static Future<SellOrderDetailProduct> fetchSellOrderDetail({
    required String orderId,
    required String customerId,
  }) async {
    final token = await TokenHelper.getToken();
    final url = Uri.parse('$sellOrderProductDetails?OrderId=$orderId&CustomerId=$customerId',
    );
    ...
    if (response.statusCode == 200) {
      ...
      return SellOrderDetailProduct.fromJson(data);
    } else {
      throw Exception('Failed to fetch sell order details');
    }
  }
