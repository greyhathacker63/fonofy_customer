...
class ProductAttributeBottomSheet extends StatefulWidget {
  final ProductDetailsModel product;
  final Function(String ramName, String romName, String colorName, double? sellingPrice, String ) onVariantSelected;
...
class _ProductAttributeBottomSheetState
  ...
  Future<void> _fetchColors() async {
    ...
    setState(() {
      ...
      "${selectedVariant?.ramName ?? ''} / ${selectedVariant?.romName ?? ''}";
      selectedColorIndex = _colorList.indexOf(selectedVariant!);
      selectedCondition = _getConditionFromRom(selectedVariant?.romName);
    });
  }

  String _getConditionFromRom(String? romName) {
    if (romName == "64GB") {
      return "Fair";
    } else if (romName == "128GB") {
      return "Good";
    } else if (romName == "256GB") {
      return "Superb";
    }
    return "Fair";
  }
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return OutlinedButton(
                    onPressed: () {
                      setState(() {
                        selectedCondition = _getConditionFromRom(selectRom);
                        selectedStorage = storage;
                        selectedVariant = _colorList[index];
                        selectedColorIndex = index;
                        _notifyParent();
